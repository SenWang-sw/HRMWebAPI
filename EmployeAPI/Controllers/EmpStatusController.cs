...
namespace EmployeAPI.Controllers
{
    ...
    {
        private readonly IEmpStatusService s;

        public EmpStatusController(IEmpStatusService service)
        {
            s = service;
        }
        [HttpGet("get all")]
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert(EmpStatus entity)
        {
            return Ok(await s.AddAsync(entity));
        }
        [HttpPost("Update")]
        public async Task<IActionResult> Update(EmpStatus entity)
        {
